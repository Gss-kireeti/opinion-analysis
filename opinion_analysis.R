...
if (file.exists(paste("Modi", '_stack.csv'))==FALSE) write.csv(df, file=paste("bitcoin", '_stack.csv'), row.names=F) 
stack <- read.csv(file=paste("Modi", '_stack.csv'))
stack <- rbind(stack, df)
stack <- subset(stack, !duplicated(stack$text))
write.csv(stack, file=paste("Modi", '_stack.csv'), row.names=F)
...
pos <- scan('F:\\sem-2\\Statistical Analysis\\positive-words.txt', what='character', comment.char=';')
neg <- scan('F:\\sem-2\\Statistical Analysis\\negative-words.txt', what='character', comment.char=';')
...
write.csv(scores, file=paste("Modi", '_scores.csv'), row.names=TRUE)
stat <- scores
...
write.csv(by.tweet, file=paste("Modi", '_opin.csv'), row.names=TRUE)
